...
library(dplyr)
g1 <-
  ggplot2::ggplot(Ratings_3 %>% filter(`Score Type`== "Comprehend"), aes(fill=`Sibling Order`, y=`Score`, x=`Family`)) + 
  #  facet_grid(`Score Type` ~ Family) +
...
g2 <-
  ggplot2::ggplot(Ratings_3 %>% filter(`Score Type`== "Speak"), aes(fill=`Sibling Order`, y=`Score`, x=`Family`)) + 
  #  facet_grid(`Score Type` ~ Family) +
  geom_bar(position="stack", stat="identity") +
  ggtitle("Speak") +
  theme(legend.position = "none") +
...
# Graph #1
############
# Create a new factor from Family & Score Type
Ratings_3 <- Ratings_3 %>% mutate(Family_ScoreType = factor(paste(Family, `Score Type`, sep="_")))

# Stacked Bar Graph (Family/ Score Type is on x-axis)
ggplot2::ggplot(Ratings_3, aes(fill=`Sibling Order`, y=`Score`, x=`Family_ScoreType`)) + 
  geom_bar(position="stack", stat="identity") +
  ggtitle("Comprehend and Speak Scores by Family") + 
...
ggplot2::ggplot(Ratings_3, aes(fill=`Sibling Order`, y=`Score`, x=`Family_ScoreType`)) + 
  ...
  ggtitle("Comprehend and Speak Scores by Family") + 
  coord_flip() +
  scale_x_discrete(limits = rev(levels(Ratings_3$Family_ScoreType))) +
  geom_vline(xintercept=c(2.5, 4.5, 6.5, 8.5, 10.5), linetype="solid", color = "black", size=1)
